...
// means pork cannot do its job until it is fixed.
type Status int
...
func checkProxy(cfg *config.Config) Check {
	...
	if err := cfg.ProxyError(); err != nil {
		c.Status = StatusFail
		c.Detail = fmt.Sprintf("invalid proxy config: %v (fix with 'pork proxy set' or 'pork proxy off')", err)
		return c
	}
	runtime := cfg.ProxyRuntime()
	if runtime == nil || !runtime.Enabled() {
		c.Status = StatusOK
		c.Detail = "not configured (run 'pork proxy tor' to route through Tor)"
		return c
	}
...
func providerCheck(p ProviderProbe) Check {
	...
	case p.Blocked:
		// Blocked is the site's anti-bot layer, not a broken pork: other
...
func checkEngine(cfg *config.Config, probe EngineProbe) Check {
	...
	if err != nil {
		if strings.Contains(err.Error(), "piece database is locked") {
			c.Status = StatusWarn
			c.Detail = "another pork instance appears to be running"
			return c
		}
		c.Status = StatusFail
		c.Detail = err.Error()
...
func checkHistory(cfg *config.Config, store *Store) Check {
	...
	// likely pork is never left open long enough for it to finish.

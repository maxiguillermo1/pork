// Package health records how pork's world is doing over time: whether each
// search provider still answers, and whether the swarms behind the library
// still have peers. Snapshots accumulate in ~/.pork/health.json - one per
...
// a doctor run produces, so trends aren't skewed by someone running `pork
// doctor` in a loop.
const (
	KindDaily  = "daily"
	KindDoctor = "doctor"
...
func Open(path string) *Store {
	...
	if errors.Is(s.loadErr, errCorruptHistory) {
		...
		fmt.Fprintf(os.Stderr, "pork: %s was corrupt (%v); backed up to %s.bak\n", path, s.loadErr, path)
		s.log = Log{}
		s.loadErr = nil
		return s
	}
	fmt.Fprintf(os.Stderr, "pork: could not read %s (%v); starting with an empty health history\n", path, s.loadErr)
	s.loadErr = nil
	return s
}
...
// needs to open a lock file beside health.json, which `pork doctor --record`
// may reach before anything has created ~/.pork.
...
// reloadLocked folds in a snapshot written by another pork process while we

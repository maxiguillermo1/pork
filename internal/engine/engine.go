...
func New(cfg *config.Config) (*Engine, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "timeout") {
			...
			return nil, fmt.Errorf("piece database is locked - another pork is already running "+
				"(find it with `pgrep -fl pork`, then quit or kill it). db: %s", dbPath)
		}
		...
		fmt.Fprintf(os.Stderr, "pork: piece database was corrupt; reset it (downloads will re-verify). old file: %s.corrupt\n", dbPath)
	}

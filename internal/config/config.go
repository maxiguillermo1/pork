// Package config owns ~/.pork paths and the YAML config file.
package config
...
import (
	...
	"github.com/maxiguillermo1/pork/internal/rank"
)
...
// ProxyConfig controls pork's optional strict SOCKS5 route. A configured proxy
...
type Config struct {
	...
	dir          string // ~/.pork, resolved at load time
...
// Load reads ~/.pork/config.yaml, writing the default file on first run.
...
// UserDir returns pork's per-user state directory without creating it. It is
...
func UserDir() (string, error) {
	...
	return filepath.Join(home, ".pork"), nil
}
...
// operation. Following one here could replace a file outside ~/.pork when the
...
// where people already expect their downloads, tucked into a pork/ subfolder
...
func defaultDownloadDir() string {
	if xdg := strings.TrimSpace(os.Getenv("XDG_DOWNLOAD_DIR")); xdg != "" {
		return filepath.Join(expandHome(xdg), "pork")
	}
	if home, err := os.UserHomeDir(); err == nil {
		return filepath.Join(home, "Downloads", "pork")
	}
	return "downloads"
}
...
func LoadFrom(dir string) (*Config, error) {
	...
	default:
		if err := yaml.Unmarshal(data, cfg); err != nil {
			...
			fmt.Fprintf(os.Stderr, "pork: %s was invalid YAML; backed up to %s.bak and reset to defaults\n", path, path)
		}
	}
...
func LoadReadOnlyFrom(dir string) (*Config, error) {
	...
	if err := cfg.initProxyRuntime(); err != nil {
		...
		// here would make doctor and `pork proxy status` abort with a bare

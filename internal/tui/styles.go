...
// porkLogo is a clean, flat half-block wordmark - minimal and calm, in the
...
// renderLogo paints porkLogo top-to-bottom, spreading the gradient evenly
// across however many rows the wordmark has.
func renderLogo() string {
	n := len(porkLogo)
	lines := make([]string, n)
	for i, l := range porkLogo {
		idx := 0
		if n > 1 {

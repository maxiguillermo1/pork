...
// knows where the project publishes its current image, so pork resolves the
...
// from official infrastructure - squarely within pork's lawful-content
// charter.
package isos
...
type Distro struct {
	...
	Edition  string // the flavor pork resolves, e.g. "netinst · amd64"
	Blurb    string // one-line cozy description
	Homepage string

	// IndexURL is the official page pork scrapes for the current torrent.
	IndexURL string
...
// sourceForgeRSS is the small part of SourceForge's files RSS feed that pork

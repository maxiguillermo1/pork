...
import (
	...
	"github.com/maxiguillermo1/pork/internal/aggregator"
	"github.com/maxiguillermo1/pork/internal/autopilot"
	"github.com/maxiguillermo1/pork/internal/config"
	"github.com/maxiguillermo1/pork/internal/engine"
	"github.com/maxiguillermo1/pork/internal/health"
	"github.com/maxiguillermo1/pork/internal/provider"
	"github.com/maxiguillermo1/pork/internal/proxy"
	"github.com/maxiguillermo1/pork/internal/state"
	"github.com/maxiguillermo1/pork/internal/tui"
)
...
func main() {
	...
	defer func() {
		if r := recover(); r != nil {
			fmt.Fprintf(os.Stderr, "pork: fatal: %v\n", r)
			os.Exit(1)
		}
	}()
...
func printHelp() {
	fmt.Print(`pork - terminal torrent search and download

Usage:
  pork [--evil] [-d DIR]
  pork autopilot [--dry-run] [--headless] [--evil] [-n N] [-d DIR] "query"
  pork doctor [--record] [--engine] [--proxy-check] [-d DIR]
  pork proxy tor|set [SOCKS5_URL]|off|status
  pork --version
...
Flags:
  -d, --download-dir DIR   where to save downloads (default: your OS
                           Downloads folder, ~/Downloads/pork)
      --evil               evil mode: downloads never seed after they finish
                           (applies to new downloads this session only)

The interactive UI stores config and state under ~/.pork and downloads into
...
// which makes `pork doctor` usable as a cron canary.
...
func runProxy(args []string) error {
	if len(args) == 0 {
		return errors.New("usage: pork proxy tor|set [SOCKS5_URL]|off|status")
	}
	switch args[0] {
	case "tor":
		if len(args) != 1 {
			return errors.New("usage: pork proxy tor")
		}
	...
	case "off":
		if len(args) != 1 {
			return errors.New("usage: pork proxy off")
		}
		...
		if update.Changed {
			fmt.Println("proxy disabled; pork will use its normal direct mode next launch")
		} else {
	...
	case "status":
		if len(args) != 1 {
			return errors.New("usage: pork proxy status")
		}
...
func proxyURLFromArgs(args []string) (string, error) {
	...
	case 0:
		if !term.IsTerminal(os.Stdin.Fd()) {
			return "", errors.New("run 'pork proxy set SOCKS5_URL' or enter a credentialed URL from an interactive terminal")
		}
	...
	case 1:
		...
		if runtime.HasCredentials() {
			return "", errors.New("for a credentialed proxy, run 'pork proxy set' and enter the URL privately")
		}
		return raw, nil
	default:
		return "", errors.New("usage: pork proxy set [SOCKS5_URL]")
	}
}
...
func saveProxy(raw string) error {
	...
	fmt.Println("verify it with: pork doctor --proxy-check")
	return nil
}
...
func printProxyStatus() error {
	...
	if err := cfg.ProxyError(); err != nil {
		return fmt.Errorf("proxy config is invalid: %v (fix with 'pork proxy set SOCKS5_URL' or 'pork proxy off')", err)
	}
	runtime := cfg.ProxyRuntime()
	if runtime == nil || !runtime.Enabled() {
		fmt.Println("proxy: not configured (run 'pork proxy tor' to route through Tor)")
		return nil
	}
...
func maybeCheckHealth(parent context.Context, cfg *config.Config, store *health.Store, providers []provider.Provider, eng *engine.Engine, warmup time.Duration) (stop func()) {
	...
	go func() {
		...
		defer func() {
			if r := recover(); r != nil {
				fmt.Fprintf(os.Stderr, "pork: background health check panicked: %v\n", r)
			}
		}()
		if warmup > 0 {
			select {
		...
		if _, err := health.Run(runCtx, health.KindDaily, providers, eng.Snapshots(), cfg.SearchTimeout(), store); err != nil && runCtx.Err() == nil {
			fmt.Fprintf(os.Stderr, "pork: background health check: %v\n", err)
		}
	}()
	return func() {
		cancel()
...
func runAutopilot(args []string) error {
	...
	dlDir := fs.String("download-dir", "", "download directory (default: your OS Downloads/pork)")
	fs.StringVar(dlDir, "d", "", "shorthand for --download-dir")
	...
	if raw == "" {
		return errors.New(`usage: pork autopilot [-n N] [--dry-run] [--headless] "query"`)
	}
